...
#           conn.send(cx*0.3)
            ...
            if i>3:
                i=0
            cx=median_filter(arr_error)
            MotorTurnOnRide(cx*0.02*13,13)
        else:
    ...
    while(y>target_y):
        ...
        if len(contours) > 1:
            ...
            if M['m00'] == 0:
                continue
            cx = int(M['m10']/M['m00'])
            Kp=1#cv2.getTrackbarPos("Kp",'frame')/100
            target=450#cv2.getTrackbarPos("Target_posiiton",'frame')
            Kp2=0.13#cv2.getTrackbarPos("Kpd",'frame')/100
            cx=cx-target
            cx=cx*Kp+(cx-arr_error_y)*Kp2
    ...
    while ((time()-start_time)<tim):
        ...
        if len(contours) > 1:
            ...
            if i>3:
                i=0
            cx=median_filter(arr_error)
            MotorTurnOnRide(cx*speed*0.02,speed)
        else:
            MotorTurnOnRide(cx*0.2,10)
            print ("I don't see the line")
...
red_stop(0,13)
turn90Right(25)
go_until_right(1.6,0.80)
goToDist(10,20)
turn90left(25)
go_until_up(2.06,1.5)
red_stop(0,10)
turnDegRight(55,25)
circle_time(7,13)
circle_stop(13)
...
red_stop(0,10)
turn90Right(25)
go_until_down(4.79,3.05)
red_stop(0,10)
turn90Right(25)
#одностороннее движение
go_one_way(2.69,0.85)
goToDist(30,20)
turn90Right(25)
